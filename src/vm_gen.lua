...
function VmGen.generate(proto, revmap, key, nonce, utils)
    ...
    LF("        for _,_layer in ipairs(_materials) do if not _all_air then break end for _,_row in ipairs(_layer) do if not _all_air then break end for _,_mat in ipairs(_row) do if _mat~=_enum.Material.Air then _all_air=false break end end end end")
