...
function VmGen.generate(proto, revmap, key, nonce, utils)
    ...
    LF("        local %s = %s.Size.Y * 0.5", atCheckVars[22], atCheckVars[14])
    LF("        return %s and math.abs(%s.Position.Y - (%s.Position.Y + %s)) <= 0.1", atCheckVars[18], atCheckVars[18], atCheckVars[14], atCheckVars[22])
    LF("    end)")
