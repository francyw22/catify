...
function VmGen.generate(proto, revmap, key, nonce, utils)
    ...
    LF("        %s.Size = Vector3.new(10, 10, 10)", atCheckVars[14])
    ...
    LF("        local %s = %s:Raycast(%s.Position + Vector3.new(0, 10, 0), Vector3.new(0, -20, 0), %s)", atCheckVars[18], atCheckVars[13], atCheckVars[14], atCheckVars[17])
    LF("        return %s and math.abs(%s.Position.Y - (%s.Position.Y + 5)) <= 0.1", atCheckVars[18], atCheckVars[18], atCheckVars[14])
    LF("    end)")
    ...
    LF("    if not (%s and tostring(%s):find(%s, 1, true)) then return false end", atCheckVars[21], atCheckVars[21], _obfLitStr("valid member"))
    LF("    return true")
    LF("end)")
