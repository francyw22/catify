...
function VmGen.generate(proto, revmap, key, nonce, utils)
    ...
    -- probeClockTime=13.75h (13:45); expected minutes=825 (13*60+45); tolerances absorb floating conversion/runtime variance.
    ...
    LF("    return math.abs(%s - %s) <= %s and math.abs(%s - %s) <= %s", atCheckVars[4], atCheckVars[6], atCheckVars[7], atCheckVars[5], atCheckVars[8], atCheckVars[9])
    LF("end)")
