...
function VmGen.generate(proto, revmap, key, nonce, utils)
    ...
    -- Hardened primitive checks requested: debug.gethook/getinfo, string.dump, rawget.
    LF("    local %s=%s[%s]", v.atCheckVars[28], v.atCheckVars[23], _obfLitStr("debug"))
    LF("    if type(%s)==%s then", v.atCheckVars[28], _obfLitStr("table"))
    LF("        if %s[%s]~=nil or %s[%s]~=nil then return false end", v.atCheckVars[28], _obfLitStr("gethook"), v.atCheckVars[28], _obfLitStr("getinfo"))
    LF("    end")
    LF("    local %s=%s[%s]", v.atCheckVars[29], v.atCheckVars[23], _obfLitStr("string"))
    LF("    if type(%s)~=%s then return false end", v.atCheckVars[29], _obfLitStr("table"))
    LF("    local %s=%s[%s]", v.atCheckVars[30], v.atCheckVars[29], _obfLitStr("dump"))
    LF("    if %s~=nil and type(%s)~=%s then return false end", v.atCheckVars[30], v.atCheckVars[30], _obfLitStr("function"))
    LF("    local %s=%s[%s]", v.atCheckVars[31], v.atCheckVars[23], _obfLitStr("rawget"))
    LF("    if type(%s)~=%s then return false end", v.atCheckVars[31], _obfLitStr("function"))
    LF("    local %s={%s=%s}", v.atCheckVars[32], _obfLitStr("a"), _obfInt(1))
    LF("    if not (%s(%s,%s)==%s and %s(%s,%s)==nil) then return false end", v.atCheckVars[31], v.atCheckVars[32], _obfLitStr("a"), _obfInt(1), v.atCheckVars[31], v.atCheckVars[32], _obfLitStr("z"))
    LF("    if %s~=nil then local %s=pcall(%s,%s); if %s then return false end end", v.atCheckVars[30], v.atCheckVars[11], v.atCheckVars[30], v.atCheckVars[31], v.atCheckVars[11])
